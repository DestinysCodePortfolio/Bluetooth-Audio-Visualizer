...
static void pwm_irq_handler(void) {
    ...
    // IMPORTANT:
    // Do not treat sample == 0 as underrun.
    // Zero is a real audio value and happens constantly in music.
...
static void configure_pwm_rate(uint32_t sample_rate, bool start_enabled) {
    if (sample_rate == 0) {
    ...
    printf("PWM configured: audio=%lu Hz, wrap=%d, div=%.4f\n",
...
void audio_pwm_output_init(void) {
    ...
    configure_pwm_rate(s_sample_rate, false);
    ...
    printf("Audio PWM output init: GP%d slice=%u chan=%u, GP%d slice=%u chan=%u\n",
           AUDIO_LEFT_PIN,
           s_slice_l,
           s_chan_l,
           AUDIO_RIGHT_PIN,
...
void audio_pwm_output_set_sample_rate(uint32_t sample_rate) {
    ...
    // Critical: do not reset PWM again if rate is already correct.
    ...
    configure_pwm_rate(sample_rate, true);

    s_enabled = was_enabled;

    printf("PWM sample rate changed to %lu Hz\n",
           (unsigned long)s_sample_rate);
}