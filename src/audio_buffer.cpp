...
void audio_buf_push(const int16_t *samples, uint32_t num_samples, AudioSource source) {
    ...
    for (uint32_t i = 0; i < num_samples; i++) {
        ...
        if (samples_available < AUDIO_BUFFER_SIZE) samples_available++;
    }
...
uint32_t audio_buf_copy_latest(int16_t *out, uint32_t max_samples) {
    ...
    uint32_t start = (write_index >= count)
                   ? write_index - count
                   : AUDIO_BUFFER_SIZE + write_index - count;
...
AudioSource audio_buf_get_source(void) { return current_source; }
bool        audio_buf_has_data(void)   { return has_data; }
